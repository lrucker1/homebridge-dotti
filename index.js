...
var Service, Characteristic, DottiIcon, Storage;
...
module.exports = function(homebridge) {
    ...
    // We can run without storage.
    try {
        Storage = require('node-persist');
    } catch (e) {
        console.log("Could not load 'node-persist'. Make sure it is installed and NODE_PATH is set. Dotti state will not be saved.");
    }
...
function DottiAccessory(log, config) {
    ...
    this.brightness = 100;
    this.icon = 0;
    this.showingColor = false;
    accessory = this;

    // Store the values as they change so we can reload them on restart.
    // Use the internally cached values for get requests.
    // It can get out of sync if the user physically changes it, but there's no read option.
    if (Storage != null) {
        Storage.init({
            logging: true
        }).then(function() {
            ...
            if (icon) {
                ...
                Storage.setItemSync('icon', accessory.icon);
            }
            accessory.log.debug('icon state: ' + icon);
        }).then(function() {
            return Storage.getItem('showingColor');
        }).then(function(showingColor) {
            if (showingColor) {
                accessory.showingColor = showingColor;
            } else {
                Storage.setItemSync('showingColor', accessory.showingColor);
            }
        }).then(function() {
            return Storage.getItem('hue');
        }).then(function(hue) {
            if (hue) {
                accessory.hue = hue;
            } else {
                Storage.setItemSync('hue', accessory.hue);
            }
        }).then(function() {
            return Storage.getItem('saturation');
        }).then(function(saturation) {
            if (saturation) {
                accessory.saturation = saturation;
            } else {
                Storage.setItemSync('saturation', accessory.saturation);
            }
        }).then(function() {
            ...
            if (brightness) {
                ...
                Storage.setItemSync('brightness', accessory.brightness);
            }
        }).catch(function(err) {
            console.error(err);
        });
...
DottiAccessory.prototype.setPowerState = function(powerState, callback) {
    ...
    // Toggle 'on' will show the default icon.
 ...
 * which is really just "no lit pixels". Some scene-building UIs assume that
 * a light must be "on" to participate in a scene.
 */
DottiAccessory.prototype.loadIcon = function(value, callback) {
    ...
    if (Storage != null) {
        ...
        Storage.setItem('showingColor', false);
    }
}
    ...
    if (this.iconCallbacks.length > 1) {
        ...
        this.loadIcon(value, function(err) {
            ...
            if (this.pendingIcon != null) {
                this.loadIcon(this.pendingIcon);
                this.pendingIcon = null;
            }
            this.log.debug("Sending result to " + this.iconCallbacks.length + " queued callbacks");
            this.iconCallbacks.forEach(function(queuedCallback, index) {
                queuedCallback(err);
            });
...
// Don't change showingColor for brightness; some UIs change that when they turn it 'off'.
    ...
    if (this.rgbCallbacks.length > 1) {
        ...
        this.dottiDevice.showRGB(rgb, function(err) {
            ...
            if (this.pendingRGB != null) {
                this.dottiDevice.showRGB(this.pendingRGB);
                this.pendingRGB = null;
            }
            this.log.debug("Sending result to " + this.rgbCallbacks.length + " queued callbacks");
            this.rgbCallbacks.forEach(function(queuedCallback, index) {
                queuedCallback(err);
            });
            this.log.debug("Clearing callback queue");
            this.rgbCallbacks = [];
            if (Storage != null) {
                Storage.setItem('hue', this.hue);
                Storage.setItem('saturation', this.saturation);
                Storage.setItem('brightness', this.brightness);
                Storage.setItem('showingColor', this.showingColor);
            }
        }.bind(this))
    }
}
...
DottiAccessory.prototype.scanForDotti = function() {
    /*
     * Noble UUID filtering can be inconsistent across platforms.
     * If there's a device ID, allow all devices through and filter by that instead.
     */
    var filter = (this.address == null) ? [DottiServiceUuid] : [];
    Noble.startScanning(filter, false);
}
...
DottiAccessory.prototype.restorePrevious = function(callback) {
    if (this.preIdentifyValues.showingColor) {
        var rgb = this.hsv2rgb(this.preIdentifyValues.hue, this.preIdentifyValues.saturation, this.preIdentifyValues.brightness);
        this.dottiDevice.showRGB(rgb, callback);

    } else {
        this.setIcon(this.preIdentifyValues.icon, callback);
    }
}
...
DottiAccessory.prototype.identify = function(callback) {
    ...
    this.preIdentifyValues = {
        showingColor: this.showingColor,
        hue: this.hue,
