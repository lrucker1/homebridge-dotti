...
module.exports = function(homebridge) {
	...
	try {
        ...
        console.log("Could not load 'node-persist'. Make sure NODE_PATH is set. Dotti state will not be saved.");
	}
...
DottiAccessory.prototype.setPowerState = function(powerState, callback) {
    ...
    // If we're turning it on and the last icon is not 'off', show that icon.
    if (powerState && this.icon > 0) {
        this.dottiDevice.loadFavoriteIcon(this.icon, function(err) {
            callback(err);
         });
   } else {
...
DottiAccessory.prototype.nobleConnected = function(error, accessory) {
    ...
    // discoverServices is broken (more inconsistent case checking?. Just fetch everything, there's not that much of it.
